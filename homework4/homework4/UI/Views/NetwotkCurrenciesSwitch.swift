...
protocol NetwotkCurrenciesSwitchDelegate: AnyObject {
    func switchNetFilter(isFilterOn: Bool)
}

final class NetwotkCurrenciesSwitch: UIView {
    private let stackView = UIStackView()
    private let filterLabel = UILabel()
    private let filterSwitch = UISwitch()
    private let filterLabelText = "Show only from net"
    
    weak var delegate: NetwotkCurrenciesSwitchDelegate?
...
private extension NetwotkCurrenciesSwitch {
    func addSubviews() {
...
private extension NetwotkCurrenciesSwitch {
    ...
    func handleFilterSwitchValueChange() {
        delegate?.switchNetFilter(isFilterOn: filterSwitch.isOn)
    }
}
